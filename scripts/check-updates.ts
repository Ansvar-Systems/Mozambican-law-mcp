#!/usr/bin/env tsx
/**
 * Mozambican Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://portaldogoverno.gov.mz';
const PORTAL_NAME = 'Mozambique Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/mozambican-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Mozambican Law MCP — Data Freshness Check');
