/**
 * search_eu_implementations — Search EU directives/regulations that have Mozambican implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_mozambican_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  mozambican_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as mozambican_statute_count
  ...
  if (input.has_mozambican_implementation) {
    sql += ' HAVING mozambican_statute_count > 0';
  }
